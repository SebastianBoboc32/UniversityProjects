...
public class programController {
    ...
    @FXML
    private TableColumn<Pair<Integer,Integer>,Integer>latchTableLocation;

    @FXML
    private TableColumn<Pair<Integer,Integer>,Integer>latchTableValue;
    ...
    public void setController(MyContoller controller) {
    ...
    public void initialize() {
        ...
        latchTableLocation.setCellValueFactory(p-> new SimpleIntegerProperty(p.getValue().first).asObject());
        latchTableValue.setCellValueFactory(p->new SimpleIntegerProperty(p.getValue().second).asObject());
    }
    ...
    private void populateHeapTableView() {
        ...
        MyIHeap<Integer,Value> heap = programState.getHeap_table();
    ...
    private void populateLatchTableView(){
        PrgState programState = getCurrentProgramState();
        MyILatchTable<Integer,Integer> latchTbl = programState.getLatchTable();
        ArrayList<Pair<Integer,Integer>>latchTableEntries = new ArrayList<>();

        for(Map.Entry<Integer,Integer> entry:latchTbl.getContent().entrySet()){
    ...
    private void populateOutputListView() {
        ...
        List<String> outputList = Objects.requireNonNull(programState).getOutput().get_all();
        int index;
    ...
    private void populateFileTableListView() {
        PrgState programState = getCurrentProgramState();
        List<String> files = new ArrayList<>(Objects.requireNonNull(programState).getFile_table().getContent().keySet());
    ...
    private void populateSymbolTableView() {
        PrgState programState = getCurrentProgramState();
        MyIDictionary<String, Value> symbolTable = Objects.requireNonNull(programState).getSymbols_table();
    ...
    private void populateExecutionStackListView() {
        ...
        if (programState != null)
            for (IStmt statement: programState.getExecution_stack().getReversed()) {
